    ...
    public function updateView() { //update the VIEW based on the users page selection
        if (isset($this->getArray['pageID'])) { //check if a page id is contained in the URL
            switch ($this->getArray['pageID']) {
                ...
                case "register":
                    //get the model
                    $register = new Register($this->postArray,$this->pageTitle, strtoupper($this->getArray['pageID']),$this->db,$this->user, 'LECTURER');
                    $navigation = new Navigation($this->user, $this->getArray['pageID']);
                    ...
                    case "application":
                    //get the model
                    $application = new Application($this->postArray,$this->pageTitle, strtoupper($this->getArray['pageID']),$this->db,$this->user, 'LECTURER');
                    $navigation = new Navigation($this->user, $this->getArray['pageID']);
                ...
                case 'login':                    
                    ...
                    if(isset($this->postArray['btnLogin'])){  //check that the login button is pressed
                        $this->loggedin=$this->user->login($this->postArray['userID'], $this->postArray['studentPassword']);                       
